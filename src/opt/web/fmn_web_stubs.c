...
static int fmn_web_printf_unit_eval(struct fmn_web_printf_unit *unit,const char *src) {
  unit->align=1;
  unit->pad=' ';
  unit->size=0;
  unit->precision=-1;
...
static int fmn_web_printf_unit_execute(char *dst,int dsta,struct fmn_web_printf_unit *unit,va_list *vargs) {
...
int fprintf(FILE *f,const char *fmt,...) {
  ...
  while (*fmt) {
    if (ok&&(*fmt=='%')) {
      struct fmn_web_printf_unit unit;
      int srcerr=fmn_web_printf_unit_eval(&unit,fmt);
      if (srcerr>0) {
        int dsterr=fmn_web_printf_unit_execute(tmp+tmpc,(int)sizeof(tmp)-tmpc,&unit,&vargs);
        if (dsterr<0) {
          ok=0;
        } else {
          tmpc+=dsterr;
  ...
  if (tmpc>=sizeof(tmp)) tmpc=sizeof(tmp)-1;
  tmp[tmpc]=0;
